...
def main():
    ...
    pulsarnet_cpus = int(config_slurm['PulsarNet_Search']['CPUS_PER_TASK'])
    ...
    pulsarnet_wall_clock = config_slurm['PulsarNet_Search']['WALL_CLOCK_TIME']
    ...
    pulsarnet_ram = config_slurm['PulsarNet_Search']['RAM_PER_JOB']
    ...
    pulsarnet_partition = config_slurm['PulsarNet_Search']['JOB_PARTITION']

    # Edit: For Pulsarnet, I will try implementing the logic that if a gpu is not available then run on cpu.
    ...
    # Edit: Try accepting these pulsarnet parameters from the pm_config file
    search_with_pulsarnet = 1#int(pm_config.search_with_pulsarnet)
    pulsarnet_flo = 40.0#int(pm_config.pulsarnet_flo)
    pulsarnet_fhi = 1000.0#int(pm_config.pulsarnet_fhi)
    ...
    dir_pulsarnet_search = os.path.join(pm_config.root_workdir, cluster, epoch, beam, "03_PULSARNET_SEARCH")

    singularity_image = config_slurm['Singularity_Image']['Singularity_Image_Path']
    singularity_image_pulsarnet = config_slurm['Singularity_Image']['Singularity_Image_PulsarNet_Path']
    ...
    pulsarnet_code_directory = config_slurm['Singularity_Image']['PULSARNET_CODE_DIRECTORY_ABS_PATH']
    ...
    with open('slurm_jobs_%s.sh' %observation_name_no_extension, 'w') as f:
            ...
            for j in range(len(time_segments)):
                ...
                if time_segments[j] == 'full':
                    ...
                    for dm in dm_loop_trials:
                        ...
                        if search_with_pulsarnet == 1:
                            working_dir_search_pulsarnet_search = os.path.join(dir_pulsarnet_search, observation_name_no_extension, time_segments[j], chunk_label, 'DM%.2f' %dm)
                            #working_dir_search = os.path.join(dir_jerk_search, observation_name_no_extension, time_segments[j], chunk_label, 'DM%.2f' %(dm))
                            f.write('###################################### Running PulsarNet Search on Observation: %s, segment: %s chunk: %s    ##################################################################' %(os.path.basename(dat_file), time_segments[j],chunk_label) + '\n')
                            # Edit: Create a slurm job for accel search, right now just do it with CPU, implement GPU later.
                            search_script = '''search=$(sbatch --parsable --job-name=PN --dependency=afterok:$dedisp --output=$logs/%s_PN_%s_%s_%s_%s_DM%.2f.out --error=$logs/%s_PN_%s_%s_%s_%s_DM%.2f.err -p %s --export=ALL --cpus-per-task=%d --time=%s --mem=%s --wrap="%s/PERIODICITY_SEARCH_AND_COPY_BACK_PULSARNET.sh %s %s %s %s %d %d %s %s %s %s %d %s %s")''' \
                                    %(cluster, epoch, beam, time_segments[j], chunk_label, dm, cluster, epoch, beam, time_segments[j], chunk_label, dm, pulsarnet_partition, pulsarnet_cpus, pulsarnet_wall_clock, pulsarnet_ram, cwd, singularity_image, mount_path, code_directory, dat_file, pulsarnet_flo, pulsarnet_fhi, working_dir_search_pulsarnet_search, output_dir, time_segments[j], chunk_label, gpu_accel_search_flag, singularity_image_pulsarnet, pulsarnet_code_directory)
                            f.write(search_script + '\n' + '\n')
                            f.write('slurmids="$slurmids:$search"' + '\n')
                            f.write('check_job_submission_limit' + '\n')
                            pulsarnet_search_beam_count+=1    
                        else:
                    ...
                    for k in range(nchunks):
                        ...
                        for dm in dm_loop_trials:
                            ...
                            if search_with_pulsarnet == 1:
                                working_dir_search_pulsarnet_search = os.path.join(dir_pulsarnet_search, observation_name_no_extension, time_segments[j], chunk_label, 'DM%.2f' %dm)
                                #working_dir_search = os.path.join(dir_jerk_search, observation_name_no_extension, time_segments[j], chunk_label, 'DM%.2f' %(dm))
                                f.write('###################################### Splitting timeseries and Running PulsarNet Search on Observation: %s, segment: %sm chunk: %s    ##################################################################' %(os.path.basename(dat_file), time_segments[j],chunk_label) + '\n')
                                # Edit: Create a slurm job for accel search, right now just do it with CPU, implement GPU later.
                                search_script = '''search=$(sbatch --parsable --job-name=PN --dependency=afterok:$dedisp --output=$logs/%s_PN_%s_%s_%s_%s_DM%.2f.out --error=$logs/%s_PN_%s_%s_%s_%s_DM%.2f.err -p %s --export=ALL --cpus-per-task=%d --time=%s --mem=%s --wrap="%s/PERIODICITY_SEARCH_AND_COPY_BACK_PULSARNET.sh %s %s %s %s %d %d %s %s %s %s %d %s %s")''' \
                                        %(cluster, epoch, beam, time_segments[j], chunk_label, dm, cluster, epoch, beam, time_segments[j], chunk_label, dm, pulsarnet_partition, pulsarnet_cpus, pulsarnet_wall_clock, pulsarnet_ram, cwd, singularity_image, mount_path, code_directory, dat_file, pulsarnet_flo, pulsarnet_fhi, working_dir_search_pulsarnet_search, output_dir, time_segments[j], chunk_label, gpu_accel_search_flag, singularity_image_pulsarnet, pulsarnet_code_directory)
                                f.write(search_script + '\n' + '\n')
        ...
        expected_pulsarnet_search_files = pulsarnet_search_beam_count
        ...
        if expected_pulsarnet_search_files > 0:
            f.write('###################################### Loop to check PulsarNet-search progress ##################################################################' + '\n')
            f.write('  # Count the number of files matching *PulsarNet.txt in the directory\n')
            f.write('while true; do\n')
            cmds = "found_files=$(find %s -name '*PulsarNet.txt' | wc -l)" % (root_dir_check_progress)
            f.write('  ' + cmds + '\n')

            f.write('  percentage=$(echo "scale=2; ($found_files / %d) * 100" | bc)\n' % expected_pulsarnet_search_files)
            f.write('  num_hashes=$(echo "($found_files * 50)/%d" | bc)\n' % expected_pulsarnet_search_files)
            f.write('  bar=$(printf "%-${num_hashes}s" "#")\n')
            f.write('  bar=${bar// /#}\n')
            ...
            f.write('  # If the number of found files is not what is expected, sleep and check again\n')
            f.write('  if [ $found_files -lt %d ]; then\n' % expected_pulsarnet_search_files)
            f.write('    echo -ne "Date: $(date), PulsarNet Search Progress: [${bar}] $percentage%% \\r" >> $progress_file\n')
            f.write('    echo "Date: $(date), PulsarNet Search Progress: [${bar}] $percentage%%" >> $progress_file\n')
            f.write('    echo -ne "Date: $(date), PulsarNet Search Progress: [${bar}] $percentage%% \\r"\n')
            f.write('    sleep 30m\n')
            f.write('  else\n')
            f.write('    echo "PulsarNet Search completed."\n')
            f.write('    echo "PulsarNet Search completed." >> $progress_file\n')
            f.write('    break\n')
            f.write('  fi\n')
            f.write('done\n')
        ...
        # Edit: Stop at this point right now, coz sifting isnt yet implemented for PulsarNet
